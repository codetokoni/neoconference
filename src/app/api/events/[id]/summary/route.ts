...
//        Persists to event.summary so subsequent GETs are instant.
...
export async function POST(
  ...
  if (!apiKey) {
    return NextResponse.json(
      { error: "openai_not_configured", hint: "Set OPENAI_API_KEY in env" },
      { status: 503 }
    );
  }

  // Gather context: chat + transcripts.
...
");
  ...
  const transcriptText = transcripts && typeof (transcripts as any).text === "string"
    ? (transcripts as any).text
    : "";
...
";
  ...
  if (context.trim().length < 50) {
    return NextResponse.json(
      { error: "no_content", hint: "No transcript or chat to summarize yet" },
      { status: 422 }
    );
  }
  ...
  try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a concise meeting summarizer. Given a transcript and chat, produce: " +
              "(1) a 2-3 sentence overview, " +
              "(2) up to 5 key decisions, " +
              "(3) up to 5 action items (with assignee if mentioned). " +
              "Output as Markdown with H3 section headers. Be terse.",
          },
    ...
    if (!r.ok) {
      ...
      return NextResponse.json(
        { error: "openai_failed", status: r.status, detail: errBody.slice(0, 400) },
        { status: 502 }
      );
    }
