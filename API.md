...
The NeoConference API lets you create meetings, mint guest join tokens, and access
events, replays and recordings programmatically.
...
Create and revoke keys from your dashboard at **Dashboard -> Developers -> API Keys**.
Keys are shown only once at creation time; store them securely. We store only a
hash of each key, never the raw value.
...
`X-RateLimit-Reset` headers. Exceeding the limit returns `429 rate_limited`.
